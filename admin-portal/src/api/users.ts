...
import { AccountApi, AccountRepresentation, CredentialRepresentation, ClientRepresentation, DeviceRepresentation, GroupRepresentation, SessionRepresentation, LinkedAccountRepresentation } from '@p2-inc/js-sdk';
...
export const Users = {
    ...
    getAccount: async function (): Promise<AccountRepresentation> {
        return accountApi.getAccount({ realm: apiRealm, userProfileMetadata: true });
    },

    updateAccount: async function (account: AccountRepresentation): Promise<void> {
        return accountApi.updateAccount({ realm: apiRealm, accountRepresentation: account });
    },
    ...
    deleteCredential: async function (credentialId: string): Promise<void> {
        return accountApi.deleteCredential({ realm: apiRealm, credentialId: credentialId });
    },
    ...
    getLinkedAccounts: async function (): Promise<Array<LinkedAccountRepresentation>> {
        return accountApi.getLinkedAccounts({ realm: apiRealm });
    },

    deleteLinkedAccount: async function (providerId: string): Promise<void> {
        return accountApi.deleteLinkedProvider({ realm: apiRealm, providerId: providerId });
    },
