...
export const getEventData = async <IDashboardEvents>() => {
    ...
    //Get events
    const events = await kcAdminClient.realms.findEvents({realm: process.env.REALM || "wizard"})
    const eventsToShow = events.filter(event => event.details?.grant_type != "client_credentials" && event.type !== "CODE_TO_TOKEN_ERROR" && event.type !== "CODE_TO_TOKEN");
...
export const getSummaryData = async <IDashboardSummaryData>() => {
    ...
    const logins = await kcAdminClient.realms.findEvents({realm: process.env.REALM!, type:"LOGIN"})
    const loginsThisWeek = logins.filter(event => event.details?.grant_type != "client_credentials" && new Date(Date.now() - 7 * 24 * 60 * 60 * 1000) <= new Date(event.time || ""))
    const loginsToday = logins.filter(event => event.details?.grant_type != "client_credentials" && new Date(event.time || "").toLocaleDateString() == new Date().toLocaleDateString() );
    const failedLogins = await kcAdminClient.realms.findEvents({realm: process.env.REALM!, type:"LOGIN_ERROR"})
