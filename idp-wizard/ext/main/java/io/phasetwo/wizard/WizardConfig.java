...
public class WizardConfig {
  ...
  public static WizardConfig createFromAttributes(KeycloakSession session) {
    ...
    Optional.ofNullable(realm.getAttribute(CONFIG_KEY("apiMode"))).ifPresent(a -> config.apiMode(a));
    Optional.ofNullable(realm.getAttribute(CONFIG_KEY("enableGroupMapping"))).ifPresent(a -> config.enableGroupMapping(a.toLowerCase().equals("true")));
    Optional.ofNullable(realm.getAttribute(CONFIG_KEY("enableLdap"))).ifPresent(a -> config.enableLdap(a.toLowerCase().equals("true")));
    Optional.ofNullable(realm.getAttribute(CONFIG_KEY("enableDashboard"))).ifPresent(a -> config.enableDashboard(a.toLowerCase().equals("true")));
    return config;
  }
