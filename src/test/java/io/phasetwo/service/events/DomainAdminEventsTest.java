...
class DomainAdminEventsTest extends AbstractOrganizationTest {
    ...
    void domainEventsTest() throws IOException {
        //verify the domain
        var domain = organization.getDomains()
                .stream()
                .findFirst()
                .orElseThrow();
        ...
        var updateEvents = getOrganizationEvents(keycloak)
                .filter(adminEventRepresentation ->
                        adminEventRepresentation.getResourceType().equals(OrganizationResourceType.DOMAIN.toString()))
    ...
    public void beforeEach() throws IOException {
        ...
        organization = createOrganization(
                new OrganizationRepresentation().name("example-org").domains(List.of("example.com", "test.org")));
    }
}
