...
public final class UserOrganizationMemberConverter {
  ...
  public static UserOrganizationMember toRepresentation(
      KeycloakSession session, RealmModel realm, OrganizationMemberModel organizationMemberModel) {
    ...
    rep.setDisableableCredentialTypes(
        user.credentialManager().getDisableableCredentialTypesStream().collect(Collectors.toSet()));
    rep.setFederationLink(user.getFederationLink());
    rep.setNotBefore(
        isLightweightUser(user)
            ? user.getCreatedTimestamp().intValue()
    ...
    rep.setOrganizationMemberAttributes(organizationMemberModel.getAttributes());
